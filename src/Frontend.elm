...
mousePositionDecoder : Decode.Decoder FrontendMsg
...
init url key =
    let
        ...
        model =
            ...
            -- WebGL Scene fields
            , meshes = WebGLScene.meshes
            , camera =
                { position = vec3 3 2 4
                , lookAt = vec3 0 0.3 0
                , up = vec3 0 1 0
                }
            , ballAnimation = WebGLScene.init
            , animationTime = 0
...
update msg model =
    case msg of
        ...
        AnimationFrame time ->
            let
                deltaTime =
                    16.67  -- Approximate 60fps

                newAnimation =
                    WebGLScene.updateAnimation deltaTime model.ballAnimation
            in
            ( { model
                | ballAnimation = newAnimation
                , animationTime = model.animationTime + deltaTime
              }
            , Cmd.none
            )
        ...
        MouseDown x y button ->
            let
                dragInfo = { startX = x, startY = y, currentX = x, currentY = y }
                newDragState =
                    case button of
                        0 -> -- Left click
                            Rotating dragInfo
        ...
        MouseMove x y ->
            case model.dragState of
                Rotating dragInfo ->
                    let
                        deltaX = (x - dragInfo.currentX) * 0.01
                        deltaY = (y - dragInfo.currentY) * 0.01

                        newRotation =
                            { x = model.cameraRotation.x + deltaY
                            , y = model.cameraRotation.y + deltaX
                            }
                    in
                    ( { model
                        | dragState = Rotating { dragInfo | currentX = x, currentY = y }
                ...
                Panning dragInfo ->
                    let
                        deltaX = (x - dragInfo.currentX) * -0.01  -- Inverted X
                        deltaY = (y - dragInfo.currentY) * 0.01   -- Inverted Y (back to normal)

                        newPan =
                            { x = model.cameraPan.x + deltaX
                            , y = model.cameraPan.y + deltaY
                            }
                    in
                    ( { model
                        | dragState = Panning { dragInfo | currentX = x, currentY = y }
                        , cameraPan = newPan
                      }
                    , Cmd.none
                    )
        ...
        MouseWheel deltaY ->
            let
                ...
                zoomSpeed = 0.001  -- Much smaller for smoother zoom
                minDistance = 2
                maxDistance = 20
                newDistance =
                    (model.cameraDistance + deltaY * zoomSpeed)
                        |> max minDistance
                        |> min maxDistance
            in
            ( { model | cameraDistance = newDistance }
            , Cmd.none
            )
